...
const Dashboard = () => {
    ...
    // Group by Dept
    const getDeptCount = (dept) => complaints.filter(c => c.department?.includes(dept)).length;
            ...
            <Row className="mb-4">
                <Col md={6}>
                    <Card className="p-3 shadow-sm border-0 h-100">
                        ...
                        <hr />
                        {['Electricity', 'Water', 'Roads', 'Waste', 'Emergency'].map(dept => (
                        ...
                        <ul className="list-unstyled small text-muted">
                            {complaints.slice(0, 5).map(c => (
                                <li key={c._id} className="mb-2">
                                    ✅ Routed <strong>{c.title}</strong> to <span className="text-primary">{c.department}</span>
                                </li>
                            ))}
                        </ul>
                    </Card>
                    ...
                    <tbody>
                        {complaints.map(c => (
                            <tr key={c._id} className={c.isEmergency ? 'table-danger' : ''}>
                                <td>{c.category}</td>
                                <td><Badge bg={c.priority === 'Emergency' ? 'danger' : 'info'}>{c.priority}</Badge></td>
                                <td>{c.title}</td>
                                <td>{c.department}</td>
                                <td>{c.status}</td>
                            </tr>
